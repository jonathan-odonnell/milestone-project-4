...
9. The back button was displaying the cached AJAX response when it reached the holiday categories, holiday details, offers or flights page.

    - Added separate URLs for the holidays and flights AJAX requests.

10. The back to top button smooth scroll was not working in Safari.

    - Implemented an alternative approach as scroll options are not compatible with Safari.

### Known Bugs

- Form validation errors are not displayed in Chrome or Edge when the first invalid input is outside of the current browser view due to an error with bootstrap.
- Form validation errors are not displayed when the cursor is in the first invalid input and the user clicks on the Paypal or Apple Pay/Google Pay button.
...
Details of how the site was deployed can be viewed [here](deployment.md).
